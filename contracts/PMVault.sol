...
contract PMVault is Ownable, ReentrancyGuard, Pausable {
    ...
    struct Plan {
        uint256 minStake;
        uint256 maxStake;
        uint256 monthlyRate; // in basis points (500 = 5%)
        uint256 minPmHold;
        uint256 maxPoolSize;
    ...
    event PlanCreated(uint256 planId, uint256 minStake, uint256 maxStake, uint256 monthlyRate, uint256 minPmHold, uint256 maxPoolSize);
    ...
    constructor(address _pmToken, address _usdtToken, address _usdcToken) {
        ...
        // Bronze: 10-1000 USDT, 5% monthly, 100k PM required, 300k max pool
        _createPlan(10 * 1e18, 1000 * 1e18, 500, 100000 * 1e18, 300000 * 1e18);
        
        // Silver: 1001-10000 USDT, 6% monthly, 300k PM required, 500k max pool
        _createPlan(1001 * 1e18, 10000 * 1e18, 600, 300000 * 1e18, 500000 * 1e18);
        
        // Gold: 10001-25000 USDT, 8% monthly, 500k PM required, 1M max pool
        _createPlan(10001 * 1e18, 25000 * 1e18, 800, 500000 * 1e18, 1000000 * 1e18);
    }
    ...
    function _createPlan(
        ...
        emit PlanCreated(planCount, minStake, maxStake, monthlyRate, minPmHold, maxPoolSize);
        planCount++;
    }
    ...
    function calculatePendingRewards(address user, uint256 stakeIndex) public view returns (uint256) {
        ...
        // Rewards = amount * (monthlyRate/10000) * (elapsed/30 days)
        uint256 dailyRate = plan.monthlyRate * 1e18 / 30 / 10000;
        uint256 daysElapsed = elapsed * 1e18 / 1 days;
        
        return (s.amount * dailyRate * daysElapsed) / 1e36;
    }
    ...
    function getGlobalStats() external view returns (
        ...
        return (totalStakedGlobal, totalRewardsDistributed, planCount);
    }
